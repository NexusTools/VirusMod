...
public class BlockArrowsTNT extends BlockTNT {
	public BlockArrowsTNT(int par1) {
		super(par1, 0);
		setBlockName("Arrows TNT");
	...
	public int getBlockTextureFromSide(int par1) {
		if(par1 == 0)
			...
			return ExpandedTNT.TNT_ARROWS_SIDES;
	}

	@Override
	public void onBlockDestroyedByExplosion(World par1World, int par2, int par3, int par4) {
		if(!par1World.isRemote) {
			EntityArrowTNTPrimed var5 = new EntityArrowTNTPrimed(par1World);
	...
	public void onBlockDestroyedByPlayer(World par1World, int par2, int par3, int par4, int par5) {
		if(!par1World.isRemote) if((par5 & 1) == 1) {
			EntityArrowTNTPrimed var6 = new EntityArrowTNTPrimed(par1World);
